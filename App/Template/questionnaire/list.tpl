...
<td valign="top" align="center">
 	<div class="main_ta_box">
         <div class="hd_t">演示文稿列表<a style="float:right;font-size:14px;" href="{url controller=Questionnaire action=DeList}">被删除的问卷</a></div>
			...
			<colgroup>
             ...
             <tr>
                 ...
                 <td style="word-break:keep-all;">
                 	<a href="{url controller=Questionnaire action=Edit qnnaid=$list[sec].id}" style="margin-right:25px;">编辑</a><a class="delBtn" href="{url controller=Questionnaire action=Del id=$list[sec].id}">删除</a><br/><br/>
                 	<a href="{url controller=Questionnaire action=RePublic qnnaid=$list[sec].id}">复制一份新问卷</a><br/><br/>
                 	{if $list[sec].status eq '0'}<a class="pubBtn f18" href="{url controller=Questionnaire action=Public id=$list[sec].id}">发布</a>
                 	{elseif $list[sec].status eq '1'}<a class="overBtn f18" href="{url controller=Questionnaire action=End id=$list[sec].id}">结束</a>
